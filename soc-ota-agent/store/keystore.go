...
func (k *Keystore) Load() error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			// Try legacy key filename for backward compatibility (mender-agent.pem)
			if k.keyName == "otapulse-agent.pem" {
				log.Debugf("Primary key file not found, trying legacy key filename")
				inf, err = k.store.OpenRead("mender-agent.pem")
				if err == nil {
					log.Infof("Loaded legacy key file: mender-agent.pem (consider migrating to otapulse-agent.pem)")
				}
			}
			if err != nil {
				if os.IsNotExist(err) {
