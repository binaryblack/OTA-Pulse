...
func (u *UpdateClient) FetchUpdate(
	...
	// If ContentLength is -1, try to read it from headers
	// This can happen with chunked transfer encoding or certain server configurations
	if contentLength < 0 {
		// Try standard Content-Length header first
		if clHeader := r.Header.Get("Content-Length"); clHeader != "" {
			if parsed, err := strconv.ParseInt(clHeader, 10, 64); err == nil {
				contentLength = parsed
				log.Infof("Content-Length from header: %d", contentLength)
			}
		}
		// Fall back to custom X-File-Size header (SoC Monitoring server)
		if contentLength < 0 {
			if fsHeader := r.Header.Get("X-File-Size"); fsHeader != "" {
				if parsed, err := strconv.ParseInt(fsHeader, 10, 64); err == nil {
					contentLength = parsed
					log.Infof("File size from X-File-Size header: %d", contentLength)
				}
			}
		}
	}
	...
	if contentLength < 0 {
		r.Body.Close()
		log.Errorf("Unknown image size. Response headers: %v", r.Header)
		return nil, -1, errors.New("Will not continue with unknown image size.")
	} else if contentLength < u.minImageSize {
		r.Body.Close()
		log.Errorf(
