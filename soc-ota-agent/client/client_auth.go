		...
		if urlErr, ok := err.(*url.Error); ok {
			...
			case x509.UnknownAuthorityError:
				...
				log.Error("If you are using a self-signed certificate, make sure it is " +
					"available locally to the OTAPulse client in /etc/otapulse/server.crt and " +
					"is configured properly in /etc/otapulse/otapulse.conf.")
				log.Error("For more information, check certificate configuration and ensure " +
					"the server certificate is trusted by the device.")
			...
			case x509.CertificateInvalidError:
				...
				case x509.Expired:
					...
					log.Error("Check that the server certificate has not expired and " +
						"the device system time is synchronized correctly.")
