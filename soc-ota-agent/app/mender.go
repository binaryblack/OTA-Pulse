...
import (
	...
	inv "github.com/binaryblack/OTA-Pulse/inventory"
...
func NewMender(config *conf.MenderConfig, pieces MenderPieces) (*Mender, error) {
	...
	// Check if SoC Monitoring authentication is enabled
	if config.UseSoCMonitoring {
		log.Info("Using SoC Monitoring authentication (X-API-Key + X-Device-ID)")

		apiKey := config.SoCAPIKey
		deviceID := config.SoCDeviceID

		if apiKey == "" {
			log.Warn("SoC Monitoring enabled but no API key configured in SoCAPIKey")
		}
		if deviceID == "" {
			log.Warn("SoC Monitoring enabled but no device ID configured in SoCDeviceID")
		}
		...
		socClient, err := client.NewSoCMonitoringClient(
		...
		m.api = socClient
		m.download = socClient
		log.Infof("SoC Monitoring client configured for server: %s, device: %s", serverURL, deviceID)
	} else {
		// Use standard Mender JWT-based authentication
