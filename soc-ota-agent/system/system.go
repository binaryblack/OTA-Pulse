...
func (s *SystemRebootCmd) Reboot() error {
	...
	// when systemd tries to unlink certain files.
	maxRetries := 5
	...
	for i := 0; i < maxRetries; i++ {
		if i > 0 {
			log.Warnf("Retrying reboot (attempt %d/%d) after error: %v", i+1, maxRetries, lastErr)
			// Sync filesystem before retry
			syscall.Sync()
			// Try to remount root as rw if needed
			if i%2 == 0 {
				log.Infof("Attempting to remount root filesystem as read-write")
				s.command.Command("mount", "-o", "remount,rw", "/").Run()
			}
		...
		if lastErr == nil {
			// Reboot command succeeded, wait for system to actually reboot
			log.Infof("Reboot command succeeded, waiting for system shutdown...")
		...
		// Check if error is filesystem-related (worth retrying)
		if !errors.Is(lastErr, syscall.EROFS) {
			// Not a read-only filesystem error, check if reboot is proceeding anyway
			log.Warnf("Reboot command returned error: %v, waiting to see if reboot proceeds...", lastErr)
			time.Sleep(10 * time.Second)
			// If we're still here, the reboot didn't happen
		}
	}
