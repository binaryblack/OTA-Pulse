...
//!
//! Monitors system health and memfaultd status, triggers reboot on failure.
...
#[derive(Parser, Debug)]
#[command(name = "memfault-watchdog")]
#[command(version = env!("CARGO_PKG_VERSION"))]
#[command(about = "Monitors system health and triggers reboot on failure")]
struct Args {
    /// Path to configuration file
    #[arg(short, long, default_value = "/etc/memfault/config.json")]
    config: String,
...
fn main() {
    ...
    info!("memfault-watchdog {} starting", env!("CARGO_PKG_VERSION"));
    ...
    while running.load(std::sync::atomic::Ordering::SeqCst) {
        ...
        if !check_memfaultd_health() {
            consecutive_failures += 1;
            warn!(
                "memfaultd check failed ({}/{})",
                consecutive_failures, args.max_failures
            );

            if consecutive_failures >= args.max_failures {
                // Try to restart memfaultd first
                info!("Attempting to restart memfaultd");
                if !restart_memfaultd() {
                    ...
                    if !check_memfaultd_health() {
                        ...
                        info!("memfaultd restart successful");
    ...
    info!("memfault-watchdog shutting down");
    hw_watchdog.close_gracefully();
}

/// Check if memfaultd is running and healthy
fn check_memfaultd_health() -> bool {
    // Check systemd service status
    let output = Command::new("systemctl")
        .args(["is-active", "--quiet", "memfaultd"])
        .status();
...
/// Restart memfaultd service
fn restart_memfaultd() -> bool {
    let output = Command::new("systemctl")
        .args(["restart", "memfaultd"])
        .status();
