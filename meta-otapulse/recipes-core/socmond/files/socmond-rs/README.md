...
- `socmond` - Main monitoring daemon
- `socmond-watchdog` - System health monitor
- `socmond-core-handler` - Kernel coredump handler

## Configuration

Configuration is stored in JSON format at `/etc/socmond/config.json`.
...
Applications can send custom metrics via Unix socket at `/run/socmond/metrics.sock`.
...
sock.connect('/run/socmond/metrics.sock')
...
```rust
use socmond::{send_metric, send_event};
...
/var/log/socmond/
├── socmond.log        # Daemon log (if not using journald)
