//! socmond-core-handler - Coredump Handler
//!
//! Called by kernel via core_pattern when a process crashes.
//! Compresses and stores coredumps for later upload by socmond.
//!
//! Usage (in core_pattern):
//!   |/usr/bin/socmond-core-handler %p %u %g %s %t %c %h %e
...
const COREDUMP_DIR: &str = "/var/lib/socmond/coredumps";
const MAX_COREDUMP_SIZE: u64 = 50 * 1024 * 1024; // 50MB
const MAX_DIR_SIZE: u64 = 500 * 1024 * 1024; // 500MB
const LOG_FILE: &str = "/var/log/socmond/coredump.log";
const SOCKET_PATH: &str = "/run/socmond/metrics.sock";
...
fn main() {
    ...
    match save_coredump(&coredump_file) {
        Ok(size) => {
            ...
            // Notify socmond
            if let Err(e) = notify_socmond(&coredump_file, &metadata_file) {
                log(&format!("Failed to notify socmond (may not be running): {}", e));
            } else {
                log("Notified socmond of new coredump");
            }
...
fn notify_socmond(coredump_file: &str, metadata_file: &str) -> io::Result<()> {
